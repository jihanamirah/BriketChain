...
app = fastapi.FastAPI(title="BricketChain API", version="1.0.0")
...
SECRET_KEY = "bricketchain-secret-key-2024"
ALGORITHM = "HS256"
...
BATCHES_DB: dict[str, BatchData] = {
    "BRK-2604-031": BatchData(
        id="BRK-2604-031",
        produk="Premium Coconut Shell Briquette",
        tanggal="April 26, 2024",
        berat="1,200 kg",
        status="QC PASSED - Automated validation (Moisture 5.2% < 7%)",
        status_class="green",
        ...
        kadar_air=5.2,
        proses=[
            BatchProcess(nama="Grinding & Sizing", durasi="2h 15m", status="done"),
            BatchProcess(nama="Mixing & Binding", durasi="1h 30m", status="done"),
            BatchProcess(nama="Molding & Pressing", durasi="3h 00m", status="done"),
            BatchProcess(nama="Oven Drying (IoT)", durasi="3h 42m", status="done"),
            BatchProcess(nama="Quality Control", durasi="45m", status="passed"),
            BatchProcess(nama="Packaging", durasi="1h 15m", status="done"),
        ],
    ...
    "BRK-2604-028": BatchData(
        id="BRK-2604-028",
        produk="Standard Coconut Shell Briquette",
        tanggal="April 24, 2024",
        berat="980 kg",
        status="QC HOLD - High Moisture Content (9.1% > 7%)",
        status_class="amber",
        ...
        kadar_air=9.1,
        proses=[
            BatchProcess(nama="Grinding & Sizing", durasi="2h 30m", status="done"),
            BatchProcess(nama="Mixing & Binding", durasi="1h 45m", status="done"),
            BatchProcess(nama="Molding & Pressing", durasi="3h 15m", status="done"),
            BatchProcess(nama="Oven Drying (IoT)", durasi="4h 10m", status="done"),
            BatchProcess(nama="Quality Control", durasi="1h 00m", status="failed"),
    ...
    "BRK-2604-019": BatchData(
        ...
        kadar_air=4.8,
        proses=[
            BatchProcess(nama="Grinding & Sizing", durasi="2h 10m", status="done"),
            BatchProcess(nama="Mixing & Binding", durasi="1h 25m", status="done"),
            BatchProcess(nama="Molding & Pressing", durasi="2h 55m", status="done"),
            BatchProcess(nama="Oven Drying (IoT)", durasi="3h 35m", status="done"),
            BatchProcess(nama="Quality Control", durasi="40m", status="passed"),
            BatchProcess(nama="Packaging", durasi="1h 10m", status="done"),
        ],
    ...
    "BRK-2604-015": BatchData(
        ...
        berat="850 kg",
        status="QC REJECTED - Automated failure (Moisture 12.5% > 7%)",
        status_class="red",
        ...
        kadar_air=12.5,
        proses=[
            BatchProcess(nama="Grinding & Sizing", durasi="2h 45m", status="done"),
            BatchProcess(nama="Mixing & Binding", durasi="2h 00m", status="done"),
            BatchProcess(nama="Molding & Pressing", durasi="3h 30m", status="done"),
            BatchProcess(nama="Oven Drying (IoT)", durasi="5h 20m", status="done"),
            BatchProcess(nama="Quality Control", durasi="1h 30m", status="failed"),
    ...
    "BRK-2604-025": BatchData(
        ...
        berat="1,100 kg",
        status="IN PRODUCTION - Oven Drying phase",
        status_class="amber",
        petani="Sumber Makmur Farm Group",
        lokasi="Kampar, Riau",
        panen="April 22, 2024",
        ...
        kadar_air=8.5,
        proses=[
            BatchProcess(nama="Grinding & Sizing", durasi="2h 20m", status="done"),
            BatchProcess(nama="Mixing & Binding", durasi="1h 35m", status="done"),
            BatchProcess(nama="Molding & Pressing", durasi="3h 05m", status="done"),
...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "BricketChain API"}
...
async def register(request: RegisterRequest) -> RegisterResponse:
    if request.username in USERS_DB:
        return RegisterResponse(success=False, message="Username sudah terdaftar")
    ...
    return RegisterResponse(success=True, message="Registrasi berhasil, silakan login")
